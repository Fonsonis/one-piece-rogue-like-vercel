...
test('all enemy identities use level thresholds with matching stats and moves regardless of account unlocks',()=>{
 const h=combatHarness();
 const failures=h.exec(`(() => {
  const failures=[];meta.charUpgrades={};maxStartLvlCap=()=>5;
 ...
 assert.equal(h.exec("makeEnemy('lucci',65).id"),'lucci-awakened');
 assert.equal(h.exec("makeChar('lucci-awakened',65).id"),'lucci');
});

test('story wilds, marine groups, bosses and ambushes create evolved enemies with existing difficulty scaling',()=>{
 const h=combatHarness();
 h.exec(`let enemies;wildEncounter=f=>{enemies=[f];};startBattle=fs=>{enemies=fs;};
...
test('saved tournament and legends combat resolve every opponent from the round level',()=>{
 for(const kind of ['tournament','legends']){
  ...
  h.exec(`accountLevel=()=>35;screenChallengeBracket=()=>{};maxStartLvlCap=()=>100;
   meta.sagaDiffWins=Object.fromEntries(SAGAS.map(s=>[s.id,{3:true}]));meta.roster=['luffy','shanks','roger'];
...
test('cooperative AI evolves while PvP keeps player selected forms',()=>{
