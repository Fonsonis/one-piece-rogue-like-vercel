...
test('25 steps per completed 1000 real metres, exactly once per milestone in each run',()=>{
 ...
 g.distance=1000*14;g.update(1/120);assert.equal(rewards.length,1);assert.equal(rewards[0].steps,25);
 advance(g,.2);g.pause();advance(g,2);g.resume();g.update(1/120);assert.equal(rewards.length,1);
 g.distance=3000*14;g.update(1/120);assert.equal(rewards.length,2);assert.equal(rewards[1].steps,50);assert.equal(rewards[1].totalSteps,75);
