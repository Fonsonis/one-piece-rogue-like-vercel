...
`public/art/cross-guild-map.png`, generado con imagegen integrado, sustituye únicamente el icono del nodo `special` en el mapa. Verificado con transparencia alfa real y a 36 × 36 píxeles en navegador. El menú utiliza la miniatura optimizada `public/art/cross-guild-menu.webp`; el mapa conserva la imagen completa.
...
Los atlas `public/art/event-atlas.webp` y `public/art/ui-atlas.webp` se generaron con la herramienta integrada imagegen el 7 de septiembre de 2026 y se sirven como WebP sin pérdida. Conservan las 36 celdas por atlas (6 × 6), el color de todos los píxeles visibles y el canal alfa originales. `event-sprites.js` sustituye las representaciones visuales, conservando los textos originales de datos y guardados. Los controles nativos de selección mantienen texto porque no admiten imágenes en sus opciones.
